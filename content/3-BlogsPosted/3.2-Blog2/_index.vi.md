...
![Kiến trúc ban đầu của Scale to Win với Application Load Balancer kết nối trực tiếp tới các máy chủ Amazon EC2 trong Auto Scaling group](./images/BlogsPosted/images/blog2/original-architecture.png)
...
![Kiến trúc được cập nhật với Amazon CloudFront và AWS WAF được đặt phía trước Application Load Balancer](./images/BlogsPosted/blog2/cloudfront-waf-architecture.png)
...
```
![Cấu hình custom header trong Amazon CloudFront origin settings](./images/BlogsPosted/cloudfront-custom-header.png)
...
NGƯỢC LẠI
    Trả về HTTP 403
```
![Listener rule của Application Load Balancer kiểm tra shared secret header trước khi chuyển tiếp yêu cầu](./images/BlogsPosted/blog2/alb-listener-secret-rule.png)
...
![AWS WAF rule cho phép machine traffic khi URI path và địa chỉ IP nguồn khớp với điều kiện đã cấu hình](./images/BlogsPosted/blog2/waf-machine-traffic-rule-1.png)

![AWS WAF rule cho phép machine traffic khi URI path và địa chỉ IP nguồn khớp với điều kiện đã cấu hình](./images/BlogsPosted/blog2/waf-machine-traffic-rule-2.png)
...
![Thứ tự đánh giá các quy tắc trong AWS WAF Web ACL](./images/BlogsPosted/blog2/waf-rule-priority.png)
